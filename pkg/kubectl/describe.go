...
func DescriberFor(kind string, c *client.Client) (Describer, bool) {
	...
	case "PersistentStorageDevice":
		return &PersistentStorageDeviceDescriber{c}, true
	case "Minion", "Node":
...
type PersistentStorageDeviceDescriber struct {
	client.Interface
}

func (d *PersistentStorageDeviceDescriber) Describe(namespace, name string) (string, error) {
	c := d.PersistentStorageDevices(namespace)
