...
func testVolume(name string, ns string) *api.PersistentVolume {
...
func TestRESTCreate(t *testing.T) {
	...
	for _, item := range table {
		_, rest := NewTestREST()
		c, err := rest.Create(item.ctx, item.volume)
		if !item.valid {
			if err == nil {
				ctxNS := api.Namespace(item.ctx)
				t.Errorf("unexpected non-error for %v (%v, %v)", item.volume.Name, ctxNS, item.volume.Namespace)
			}
			continue
		}
		if err != nil {
			t.Errorf("%v: Unexpected error %v", item.volume.Name, err)
			continue
		}
		if !api.HasObjectMetaSystemFieldValues(&item.volume.ObjectMeta) {
			t.Errorf("storage did not populate object meta field values")
		}
		if e, a := item.volume, (<-c).Object; !reflect.DeepEqual(e, a) {
...
func TestRESTDelete(t *testing.T) {
	_, rest := NewTestREST()
	volume := testVolume("foo", api.NamespaceDefault)
	c, err := rest.Create(api.NewDefaultContext(), volume)
	if err != nil {
		t.Fatalf("Unexpected error %v", err)
	}
	<-c
	c, err = rest.Delete(api.NewDefaultContext(), volume.Name)
	if err != nil {
		t.Fatalf("Unexpected error %v", err)
	}
...
func TestRESTList(t *testing.T) {
	...
	volumeA := testVolume("foo", api.NamespaceDefault)
	volumeB := testVolume("bar", api.NamespaceDefault)
	volumeC := testVolume("baz", api.NamespaceDefault)
