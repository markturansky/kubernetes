...
func TestPersistentVolumeCreate(t *testing.T) {
	persistentVolumeA := &api.PersistentVolume{
		ObjectMeta: api.ObjectMeta{Name: "foo", Namespace: api.NamespaceDefault},
	}

	persistentVolumeB := &api.PersistentVolume{
		ObjectMeta: api.ObjectMeta{Name: "foo", Namespace: api.NamespaceDefault},
	}
	nodeWithVolumeA := tools.EtcdResponseWithError{
		R: &etcd.Response{
			Node: &etcd.Node{
				Value:         runtime.EncodeOrDie(testapi.Codec(), persistentVolumeA),
				ModifiedIndex: 1,
				CreatedIndex:  1,
			},
		},
	...
	table := map[string]struct {
		...
		"normal": {
			...
			toCreate: persistentVolumeA,
			errOK:    func(err error) bool { return err == nil },
		},
		"preExisting": {
			existing: nodeWithVolumeA,
			expect:   nodeWithVolumeA,
			toCreate: persistentVolumeB,
