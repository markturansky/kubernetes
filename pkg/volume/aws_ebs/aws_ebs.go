...
func (plugin *awsElasticBlockStorePlugin) NewCreater(options volume.VolumeOptions) (volume.Creater, error) {
	if len(options.AccessModes) == 0 {
		options.AccessModes = plugin.GetAccessModes()
	}
	return plugin.newCreaterInternal(options, &AWSDiskUtil{})
}

func (plugin *awsElasticBlockStorePlugin) newCreaterInternal(options volume.VolumeOptions, manager ebsManager) (volume.Creater, error) {
	return &awsElasticBlockStoreCreater{
		awsElasticBlockStore: &awsElasticBlockStore{
			manager: manager,
			plugin:  plugin,
		},
...
type ebsManager interface {
	...
	// Creates a volume
	CreateVolume(creater *awsElasticBlockStoreCreater) (string, int, error)
	// Deletes a volume
...
func (c *awsElasticBlockStoreCreater) Provision(pv *api.PersistentVolume) error {
	volumeID, size, err := c.manager.CreateVolume(c)
	if err != nil {
		return err
	}
	pv.Spec.PersistentVolumeSource.AWSElasticBlockStore.VolumeID = volumeID
	pv.Spec.Capacity = api.ResourceList{
		api.ResourceName(api.ResourceStorage): resource.MustParse(fmt.Sprintf("%d", size)),
	}
	return nil
}

func (c *awsElasticBlockStoreCreater) NewPersistentVolumeTemplate() (*api.PersistentVolume, error) {
	// Provide dummy api.PersistentVolume.Spec, it will be filled in
	// awsElasticBlockStoreCreater.Provision()
	return &api.PersistentVolume{
		ObjectMeta: api.ObjectMeta{
			GenerateName: "pv-aws-",
			Labels: map[string]string{
				"createdby": "aws_ebs_dynamic_provisioner",
			},
