...
func makeTestController(provisionerName string) (*PersistentVolumeController, *mockControllerClient) {
	...
	provisioners := map[string]volume.ProvisionableVolumePlugin{
		provisionerName: mockVolumePlugin,
	}
...
func TestReconcileClaim(t *testing.T) {
	controller, mockClient := makeTestController("foo")
	pvc := makeTestClaim()
	...
	if mockClient.volume != nil {
		t.Errorf("Unexpected volume found in mock client.  Expected nil")
	}
	...
	if mockClient.volume == nil {
		t.Errorf("Expected to find bound volume but got nil")
	}
...
func TestReconcileVolume(t *testing.T) {
	...
	if !isAnnotationMatch(pvProvisioningRequired, pvProvisioningCompleted, mockClient.volume.Annotations) {
		t.Errorf("Expected %s but got %s", pvProvisioningRequired, mockClient.volume.Annotations[pvProvisioningRequired])
	}
}
...
func (c *mockControllerClient) WatchPersistentVolumes(labels labels.Selector, fields fields.Selector, options api.ListOptions) (watch.Interface, error) {
...
func (c *mockControllerClient) WatchPersistentVolumeClaims(namespace string, labels labels.Selector, fields fields.Selector, options api.ListOptions) (watch.Interface, error) {
	return watch.NewFake(), nil
}
