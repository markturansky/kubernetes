...
func (c *taskContext) doReclaimPolicyRetain() error {
	if err := c.setVolumeRecycleConditionSatisfied("Recycled", "Reclaim policy is Retain"); err != nil {
		return err
	}
...
func (c *taskContext) isVolumeUnbound() bool {
	return c.volume.Spec.ClaimRef == nil
}
...
func newTaskContext(host *taskHost, volume *api.PersistentVolume, claim *api.PersistentVolumeClaim) *taskContext {
	ctx := &taskContext{
		host:   host,
		volume: volume,
		claim:  claim,
	}
	ctx.volumeConditionHandlers = map[api.PersistentVolumeConditionType]func(obj interface{}) error{
		api.PersistentVolumeBound: ctx.syncVolumeBoundCondition,
	}
	ctx.claimConditionHandlers = map[api.PersistentVolumeClaimConditionType]func(obj interface{}) error{
		api.PersistentVolumeClaimBound: ctx.syncClaimBoundCondition,
	}
	return ctx
}

func newTaskHost(client controllerClient, volumeStore *persistentVolumeOrderedIndex, claimStore cache.Store, queue *workqueue.Type, plugins []volume.VolumePlugin, cloud cloudprovider.Interface) (*taskHost, error) {
	taskHost := &taskHost{
		client:      client,
...
func newWorkerTask(syncFunc func(obj interface{}) error, src interface{}) *workerTask {
	return &workerTask{
		sync: syncFunc,
		src:  src,
	}
	...
	for {
		func() {
			...
			if task, ok := obj.(workerTask); ok {
				err := task.sync(task.src)
				if err != nil {
					glog.Errorf("Error in worker task: %v %v", err, task.src)
					return
				}
			}
		}()
...
func (c *taskContext) syncVolume(obj interface{}) error {
	...
	for _, condition := range c.volume.Status.Conditions {
		if handler, exists := c.volumeConditionHandlers[condition.Type]; exists {
			glog.V(5).Infof("Queueing task for condition %s", condition.Type)
...
func (c *taskContext) syncClaim(obj interface{}) error {
	key := obj.(string)
	c.host.lock.LockKey(key)
	defer c.host.lock.UnlockKey(key)

	if c.claim == nil {
		glog.V(5).Infof("No claim to sync")
	}
	for _, condition := range c.claim.Status.Conditions {
		if handler, exists := c.claimConditionHandlers[condition.Type]; exists {
...
func NewTaskClient(c clientset.Interface, store cache.Store) taskClient {
